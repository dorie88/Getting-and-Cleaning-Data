run_analysis <- function(){
  ...
  #load column information, mean and std information
 ...
 subject <- rbind(subject_test, subject_train)
  ...
  #merge data frames to form one table
  data <- cbind(subject, X, activity)
  write.table(data, "merged_tidy_data.txt")
  
  #create a tidy data set with subject and activity, with mean and std calculation
  library(data.table)
